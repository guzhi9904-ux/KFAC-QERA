...
# Same separate dependencies already used for the old 4096 word-PPL run.
# No installation, git pull, or shared-environment changes.
...
exec "$BASE/conda_envs/qera-original-a/bin/python" -B \
  "$TOOLS_DIR/full_a_all_ranks_dual_ppl_v1.py" "$COMMAND" \
  ...
  --output-dir "$BASE/qera_diagnostics/full_a_all_ranks_dual_ppl_v1" \
  "$@"
