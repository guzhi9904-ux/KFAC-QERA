...
def softmax_row_audit(p,dp,e):
    """Native rounded probabilities need not sum to exactly one.

    For the original tensors, mu=sum(p*dp), sum(p*(dp-mu))=mu*(1-sum(p)).
    Keep the original 8eps32 rounding budget on the residual from that identity.
    Do not renormalize p or change e used by any subsequent diagnostic.
    """
    p64=p.double();dp64=dp.double();e64=e.double()
    mass=p64.sum(-1);mu=(p64*dp64).sum(-1)
    row_sum=e64.sum(-1);expected=mu*(1-mass)
    row_scale=(p64*dp64.abs()).sum(-1)
    bound=8*torch.finfo(torch.float32).eps*row_scale+1e-20
    residual=row_sum-expected;ratio=residual.abs()/bound
    require(torch.isfinite(p).all() and torch.isfinite(dp).all() and torch.isfinite(e).all(),'Nonfinite softmax tensors')
    require(bool((p>=0).all()) and bool(((mass-1).abs()<=1e-5).all()),'Softmax probabilities invalid or unnormalized')
    worst=int(ratio.argmax());ideal=p64*(dp64-mu[:,None])
    record=dict(max_signed_row_sum=float(row_sum.abs().max()),max_row_bound=float(bound.max()),
                max_probability_mass_error=float((mass-1).abs().max()),max_expected_row_sum=float(expected.abs().max()),
                max_kernel_residual=float(residual.abs().max()),max_bound_ratio=float(ratio.max()),
                old_zero_row_check_failures=int((row_sum.abs()>bound).sum()),
                worst_row=worst,worst_row_sum=float(row_sum[worst]),worst_expected_sum=float(expected[worst]),
                worst_kernel_residual=float(residual[worst]),worst_bound=float(bound[worst]),
                native_vs_fp64_absolute=float((e64-ideal).norm()),
                native_vs_fp64_relative=None if float(ideal.norm())<=1e-24 else float((e64-ideal).norm()/ideal.norm()),
                passed=bool((ratio<=1).all()))
    require(record['passed'],'Softmax signed row identity failed: '+str(record))
    return record


def centered_formula_audit(p,dp,e,delta,v,z):
    """Separate native backward validation from algebraic FP64 reassociation.

    The FP32 forward z=P@V and the FP32 backward dp=delta@V.T have distinct
    rounding errors; cancellation can amplify their scalar-dot discrepancy.
    All diagnostics below leave native e unchanged for K reconstruction.
    """
    p64=p.double();delta64=delta.double();v64=v.double()
    dp64=delta64@v64.T;z64=p64@v64
    mu64=(p64*dp64).sum(-1,keepdim=True)
    direct64=p64*(dp64-mu64)
    centered64=p64*(dp64-(delta64*z64).sum(-1,keepdim=True))
    native_center=p*(dp-(delta*z).sum(-1,keepdim=True))
    difference=native_center.double()-e.double();refnorm=float(e.double().norm())
    return dict(
        algebra_FP64=compare(centered64,direct64,1e-10,scale=float((p64*dp64).norm())),
        native_edge_vs_FP64=compare(e,direct64,1e-5,scale=float((p64*dp64).norm())),
        native_dp_vs_FP64=compare(dp,dp64,1e-5),native_z_vs_FP64=compare(z,z64,1e-5),
        FP32_reassociation_observation=dict(absolute_error=float(difference.norm()),
            relative_error=None if refnorm<=1e-24 else float(difference.norm())/refnorm,
            max_element_error=float(difference.abs().max()),reference_norm=refnorm,
            note='Different rounded GEMM paths; reported, not substituted for the algebra or native-edge checks'))
...
def probe_layer(state,delta,cached_g,residuals,device,window,layer,window_index,resources,pilot=False,full=None):
    ...
    for a in range(hq):
        ...
        with resources.timed('native_softmax_and_K_reconstruction',window=window,layer=layer,head=a):
            ...
            check=centered_formula_audit(p,dp,e,da,va,z[:,a,:])
            require(torch.count_nonzero(p.triu(1))==0 and torch.count_nonzero(e.triu(1))==0,'Masked edge contributes')
            row_check=softmax_row_audit(p,dp,e)
            require(torch.count_nonzero(e[-1])==0,'Loss-free last query has nonzero gradient')
            row_audits.append(dict(head=a,centered_formula=check,**row_check,zero_gradient_rows=int((e==0).all(-1).sum()),
