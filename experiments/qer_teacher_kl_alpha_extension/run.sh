...
RUN="$(realpath -m -- "$1")"
case "$RUN" in /data2/cck/KFAC-QERA/runs/qer_teacher_kl_alpha_extension/*) ;; *) exit 79;; esac
...
cd "$TOOLS"
"$PYTHON" -B test_analysis.py
exec "$PYTHON" -u -B extend.py --config "$TOOLS/config.json" --output "$RUN"
