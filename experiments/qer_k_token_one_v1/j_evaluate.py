import torch
from j_common import MODULES,BASELINES,TensorStore,slug,mo,require,load_record,checked_files,sha_file,commit
...
def evaluate(root,config,ident,resources,teacher,source):
    ...
    for window in range(16):
        ...
        if window==0:
            ...
            for n in MODULES:
                for method in BASELINES:
                    key=method;w=source.baseline_weight(n,method)
                    with resources.timed('baseline_KL_replay',module=n,candidate=key),teacher.deploy({n:w}):actual=teacher.scores(ids,logits)
                    oldpath,pid=source.baseline_path(n,method,0);expected=load_record(oldpath,pid)
        ...
        for n,p in zip(MODULES,targets):
            ...
            w=store.get(root/'corrections'/slug(n)/'token_joint_one.safetensors')[0]['W_deploy']
            ...
            commit(p,ident,module=n,window=window,candidate='Token-joint-one',token_hash=token_hash,freeze_hash=freeze_hash,scores=actual,repeat_absolute_error=repeat)
