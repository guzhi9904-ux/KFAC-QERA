...
import signal
import torch
from s_common import (Path,read,save_json,identity,PLAN,require,lock,load_record,checked_files,
                      Resources,ReplayTeacher,Source)
from s_assets import prepare_assets
from s_statistics import collect
from s_solve import solve
from s_evaluate import evaluate
...
def main():
    ...
    kroot=args.ko_run.resolve();root=args.output.resolve();manifest=identity(kroot);ident=manifest['identity']
    config=read(kroot/'manifest.json')['config']
    for src in (kroot,Path(config['parent_run'])):
    ...
    with lock(root):
        ...
        if (root/'complete.json').exists():checked_files(root,load_record(root/'complete.json',ident)['files']);print('SENSITIVITY_ALREADY_COMPLETE');return
        source=Source(kroot)
        ...
        try:
            ...
            collect(root,ident,resources,kroot,assets)
