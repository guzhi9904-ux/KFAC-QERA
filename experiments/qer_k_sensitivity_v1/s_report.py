...
import time
import numpy as np
from s_common import (MODULES,METHOD,BASELINES,slug,load_record,sha_file,require,mo,read,save_json,
...
def report(root,config,ident,resources,source):
    ...
    seed=int.from_bytes(hashlib.sha256(b'k-sensitivity-v1/bootstrap').digest()[:8],'little')
    ...
    timings=resources.data['timings'];stage_rows=[]
    for stage in sorted({t['stage'] for t in timings}):
        ts=[t['seconds'] for t in timings if t['stage']==stage and t['completed']]
        stage_rows.append(dict(stage=stage,completed_calls=len(ts),summed_call_seconds=sum(ts),
                               note='Overlapping GPU worker calls; sum is not wall time'))
    save_csv(root/'summary/stage_costs.csv',stage_rows)
    newer=[r for r in rows if r['method']==METHOD];wins=sum(r['gain_vs_Marginal_pp']>0 for r in newer)
    lines=['# Sensitivity-weighted Marginal K', '',
           f'唯一主问题：相同N256/rank64下，只改变A的真实梯度强度加权是否优于标准Marginal-K？本轮四层中{wins}层的实际teacher-KL点估计低于Marginal；逐层差异如下，不能只用层平均代替。',
           '', '| 模块 | A-only | Marginal | Token-joint | Sequence-one-step | Sensitivity-weighted Marginal |',
           '|---|---:|---:|---:|---:|---:|']
    for name in MODULES:
    ...
    lines+=['','本轮A=sum(||g||²xxᵀ)/sum(||g||²)，G逐元素复用原完整Marginal G；标签、真实K梯度、量化权重与原实验一致。',
            '新A与G初始化为I的Token-joint第一轮A仅差正标量，但本方法保持Marginal G，不等于Token-joint的第一轮因子对。正负结果均不能唯一归因于梯度方向、单次标签采样噪声或权重集中。',
    ...
    lines += ['', '|模块|权重有效数量|全部位置数|最大权重占总量|', '|---|---:|---:|---:|']
    for name in MODULES:
        wr=load_record(root/'statistics'/slug(name)/'weight_summary.json',ident)
        lines.append(f"|{name}|{wr['weight_effective_count']:.1f}|{wr['positions']}|{100*wr['max_w']/wr['D']:.5f}%|")
    lines += ['', '权重有效数量仅描述集中程度，不是独立文章或独立梯度样本数。没有预设等效界限，不凭区间跨零宣称与Token等效。']
    text='\n'.join(lines)+'\n'
    for name in ('READOUT.md','RESULTS.md'):(root/name).write_text(text,encoding='utf-8')
    commit(root/'verification.json',ident,passed=True,new_KL=64,reused_KL=320,pilot=read(root/'pilot/math_checks.json'),
           no_test=True,only_A_changed=True)
    ...
    print('SENSITIVITY_EXPERIMENT_COMPLETE',flush=True)
