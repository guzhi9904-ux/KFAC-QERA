...
class Tests(unittest.TestCase):
    ...
    def test_data_budget_and_scope_accounting(self):
        ...
        from fitting import candidate_keys
        config=defaults();validate(config)
        self.assertEqual(config['methods'],['Marginal','Token-joint','Sequence-one-step'])
        self.assertEqual(len(candidate_keys(config)),7)
        with self.assertRaisesRegex(RuntimeError,'Full-fit excluded'):
            validate(dict(config,methods=config['methods']+['Full-fit']))
        ...
        self.assertEqual(plan['validation_single_module_forwards'],16*2*7)
        self.assertEqual(plan['test_joint_forwards'],3*7)
