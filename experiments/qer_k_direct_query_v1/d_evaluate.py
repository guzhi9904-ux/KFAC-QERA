import torch
from d_common import MODULES,BASELINES,TensorStore,slug,mo,require,load_record,checked_files,sha_file,commit
...
def evaluate(root,config,ident,resources,teacher,source):
    ...
    for window in range(16):
        ...
        for n,p in zip(MODULES,targets):
            ...
            w=store.get(root/'corrections'/slug(n)/'direct_query.safetensors')[0]['W_deploy']
            ...
            commit(p,ident,module=n,window=window,candidate='Direct Query-Marginal',token_hash=token_hash,freeze_hash=freeze_hash,scores=actual,repeat_absolute_error=repeat)
