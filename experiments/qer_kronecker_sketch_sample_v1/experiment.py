...
class Experiment:
    def __init__(self,root,config,identity,resources,teacher=None,device='cuda:0'):
        ...
        self.quantized=read_tensors(Path(config['assets'])/'exp01/quantized'/(slug(self.name)+'.safetensors'))[0]
    ...
    def fit(self):
        ...
        for budget in PLAN['budgets']:
            ...
            for method in PLAN['methods']:
                self.boundary();t=self.raw_factors(budget,method)
                self.candidate(budget+'__'+method,t['A_raw'],t['G_raw'],budget,method)
    ...
    def evaluate(self):
        ...
        for c in range(PLAN['eval_windows']):
            ...
            for row,qp in zip(rows,qs):
                ...
                key=row['id'];sp=self.path('eval_S',key);label,lm=labels[key]
                if sp.exists():s=self.store.get(sp)[0]['S'].to(x.device)
                else:
    ...
    def pilot(self):
        ...
        result=commit(p,self.identity,passed=True,fit_only=True,fit_samples_reused=keys,full_fit_round=full_audit,
            SVD=solve_audit,Gram=gram_check,H_norm2=hnorm2,damping_expansion=expansion_check,
