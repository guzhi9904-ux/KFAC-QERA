...
class Teacher:
    ...
    def output_perturbation_kl(self,name,ids,reference,x,residual):
        target=self.model.get_submodule(name);before=mo.digest_tensor(target.weight)
        def perturb(module,args,output):
            require(torch.equal(args[0],x),'Output-perturbation target input differs')
            delta=(args[0].double()@residual.to(args[0].device).T).to(output.dtype)
            return output-delta
        handle=target.register_forward_hook(perturb)
        try:
            with torch.no_grad():actual=hidden_forward(self.model,ids)
            return self.kl(reference,actual)
        finally:
            handle.remove()
            require(mo.digest_tensor(target.weight)==before,'Output path mutated teacher')
