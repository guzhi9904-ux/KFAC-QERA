...
from portable_model import PortableModel
from bridge import CONFIG,EXP01


class Runtime(PortableModel,FullFitExperiment):
    ...
    def __init__(self,runroot,name,deadline):
        ...
        self.parent=EXP01
        self.config=dict(read(self.parent/'identity.json')['config'])
        self.config.update(gpus=2,model=CONFIG['model'],qera=str(Path(CONFIG['assets'])/'vendor'))
        self.compute_device=int(int(name.split('.')[2])>=16);torch.cuda.set_device(self.compute_device)
    ...
    def reference(self,c):
        ...
        if self.is_parent:self.replay_parent(c,reference,state['x'].detach())
