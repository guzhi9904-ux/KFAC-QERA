#!/usr/bin/env python3
"""Explicit portable pilot/formal controller: one worker spanning two RTX 4090 GPUs."""
import argparse
import fcntl
import json
import math
...
def memory_limit():
    return int(cgroup_memory()['memory.max'])
...
def accept_pilot(root,workers,elapsed):
    ...
    save_json(root/'pilot_acceptance.json',dict(passed=True,identity=read(root/'identity.json')['identity'],workers=workers,
        ...
        host_peak_measurements=sorted({r.get('cgroup_memory',{}).get('peak_measurement','unknown') for r in resources}),
        execution='one model, two GPUs, one worker',
...
def main():
    ...
    parser.add_argument('--workers',type=int,choices=[1],default=1);args=parser.parse_args()
    assert os.name=='posix'
    root=Path(args.output).resolve()
    allowed=Path(CONFIG['output_parent']).resolve()
    assert root.is_relative_to(allowed) and root!=allowed
    for protected in (HERE,Path(CONFIG['assets']),Path(CONFIG['model'])):
        assert not root.is_relative_to(protected.resolve())
    ...
    with (root/'.controller.lock').open('w') as lock:
        ...
        assert len(devices)==2 and all(devices) and len(set(devices))==2,'One worker needs two distinct visible GPUs'
