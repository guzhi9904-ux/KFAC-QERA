...
def report(root,verify=True):
    ...
    if verify:
        ...
        for role in ('fit','eval'):
            for row in index[role]:
                ...
                if role in ('fit','eval'):
                    gp=root/('cache/'+role+'_g')/(row['id']+'.safetensors');gm=read(gp.with_suffix('.json'))
                    require(sha_file(gp)==gm['file_sha256'] and gm['sample']==row,'Gradient g cache differs')
    ...
    lines=['# Kronecker sketch / sample-budget pilot','',f'完整验收：通过。L10.q_proj，固定 MXINT3 与 rank64。累计活动时间 {hours:.3f} 小时。',
